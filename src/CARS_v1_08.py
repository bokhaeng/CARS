...
# =============================================================================
def read_activity_data_csv_SK(input_dir, inter_dir, case_name, ad_file, End_date, future_case = 'NO'):
    start_time = time.time()
    print('******************************************************************')
...
# =============================================================================
def roads_grid_surrogate_inf(input_dir, inter_dir, output_dir, File_Name, link_shape_att, case_name, grid_size, 
                             grid4AQM='no', GridDesc_file='', Radius_Earth='', outGridShape = 'yes', Unit_meters = True):
    start_time = time.time()
    print('******************************************************************')
    ...
    if os.path.exists(shp_file) == True:
        ...
        if ((grid4AQM.lower() == 'yes') or (grid4AQM.lower() == 'y')) and (gridfile_name != ''):
            ...
            if (outGridShape.lower() == 'yes') or (outGridShape.lower() == 'y'):
                grid.to_file(filename = inter_dir+'/grid_{0}.shp'.format(case_name), driver='ESRI Shapefile')
...
# Function to read link level shapefile
# =============================================================================
def processing_County_shape(input_dir, inter_dir, output_dir, file_name, County_Shape_att, case_name, GridDesc_file='', Radius_Earth='', grid4AQM = 'no'):
    start_time = time.time()
    print('******************************************************************')
...
# =============================================================================
def read_emissions_factor_SK(input_dir, EmisFactor_list, inter_dir, case_name, output_dir):
    start_time = time.time()
    print('******************************************************************')
...
# =============================================================================                              
def calculate_EmisFact(Input_dir, Emissions_Factor_Table, temp_min, temp_mean, temp_max, case_name, inter_dir, Out_dir ):
    start_time = time.time()
    print('******************************************************************')
...
# =============================================================================                              
def apply_deterioration_emissions_factor_SK(input_dir, inter_dir, output_dir, case_name, STDATE, ENDATE,
...
# =============================================================================
def calc_County_Emissions(input_dir, inter_dir, output_dir, case_name, STDATE, ENDATE, 
                          temp_min, temp_mean, temp_max,
                          Emissions_Factor_DataFrame, Activity_Data_DataFrame, 
                          Roads_DataFrame, County_DataFrame, Cold_Start_input, 
                          avgSpeedDist, road_restriction, process_road_restriction='no'):   
    start_time = time.time()
    print('')
    print('******************************************************************')
...
# =============================================================================
# Function to apply the control strategy
# =============================================================================
def aplly_control(input_dir, inter_dir, output_dir, case_name, STDATE, ENDATE,
                  Control_list_file, County_Emissions_df, County_SHP_df):
    start_time = time.time()
    print('')
    print('******************************************************************')
...
#        print(ef_list[ifile])
    ...
    aux_wgt = cnt_df.county_total_WGT.loc[(cnt_df.county_total_WGT.pollutant == aux_pol),
                                          ['region_cd','geometry','vkt_weight']].reset_index(drop=True)
...
def gridded_emis_NC(input_dir, inter_dir, output_dir, case_name, STDATE, ENDATE,
                    Activity_data_DF, Temporal_profile, County_Emissions_ChemSpec,
...
def plot_figures_chart(input_dir, inter_dir, output_dir, case_name, STDATE, ENDATE,
                       County_Emissions, County_Emissions_ChemSpec, Activity_data_DF, 
                       gridded_emissions, roads_RGS, avgSpeedDist, run_period,
                       plot_figures = 'no', plot_24 = 'no', pol_list = [], species_list = [],
                       adj_scale = 0.4, color_map = None):
    print('')
    print('******************************************************************')
    ...
    if plot_figures == 'yes':
        ...
        for ipol in polls:
            ...
            ccc = Emissions_by_link.loc[(Emissions_by_link.pollutant == ipol)].reset_index(drop=True)
            ...
            ccc.plot(column='total_emis', cmap=cmap, linewidth=1.0, ax=ax,
        ...
        for ipol in species:
            ...
            fig, ax = plt.subplots(1, figsize = (13,8))
            plot_pol.plot(column='total_emis', cmap=cmap, linewidth=0.8, ax=ax,
    ...
    if plot_24 == 'yes':
        for ipol in species:
            ...
            for itime in run_period.DateTime.loc[0:24]:
                fig, ax = plt.subplots(1, figsize = (13,8))
                plot_pol.plot(column=plot_pol[itime], cmap=cmap, linewidth=0.8, ax=ax,
                              vmax = vmax)# ,edgecolor='0.8')
