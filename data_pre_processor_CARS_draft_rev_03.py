...
'''
case_name = 'Country' #'_Seoul_Old_AD'#
home_dir   = r'C:/Users/pedruzzi/OneDrive - University of North Carolina at Chapel Hill/0000_EUA_IE/001_mobile_source_inventory/CARS_source_code/CARS_test_Case_Country'
...
# =============================================================================
def read_activity_data_csv_SK(input_dir, ad_file, End_date):
    start_time = time.time()
    ad_file = ad_file
    end_date = End_date
#    now = dt.datetime.now()
...
#        activity_data = (pd.read_csv(name, sep = sep, usecols = [0,1,2,3,4,5], encoding = 'utf-8')).fillna(0)
            ...
            activity_data['count'] = activity_data.loc[:,'manufacture_date'] * 0 +1
...
#            vhc_years = pd.DataFrame({'vhc_years' : list(grouped.manufacture_date.unique())})
    ...
    gendf_list_drop = outdf.loc[outdf.region_cd.str.endswith('0000')].index.to_list()
    print('')
    ...
    print('')
    gendf = outdf.loc[outdf.region_cd.str.endswith('0000')].reset_index(drop=True)
    ...
    splitdf  = outdf.loc[:,['manufacture_date','region_cd',vhc_max_vkt]]
    ...
    count_outdf   = count_outdf.drop(index=gendf_list_drop).reset_index(drop=True)
...
#    cold_start = Cold_Start_list  
    ...
    for ipol in npol:
        ...
        AD_by_EF_df = AD_by_EF_df.groupby(['manufacture_date','region_cd','pollutant','road_type']).sum().reset_index(drop=False)
        AD_by_EF_df = AD_by_EF_df.drop(columns=['years']).sort_values(by=['region_cd','manufacture_date']).reset_index(drop=True)
    ...
    Emissions_by_yr_county2csv = Emissions_by_yr_county.copy()
...
#        print(ef_list[ifile])
    ...
    with open(output_dir+'/LOGS/'+CSlog_name, 'w') as CS_log:
        ...
        for ivhc in aux_list_CS:
            if (ivhc not in ADvhc) or (ivhc not in EFvhc):
                ...
                CS_log.write('**** WARNING ***:There is no EF for vehicle {0}\n'.format(ivhc))
    ...
    Ed_aux =   9.1 * (np.exp(0.0158 * (RVP - 61.2) + 0.0574 * \
                     (temp_min - 22.5) + 0.0614 * \
    ...
    R_vhc_count = AD_yr_vhc.vhc_count.copy()
    R_vhc_count.loc[:,list_vhc_no_gas] = R_vhc_count.loc[:,list_vhc_no_gas] * 0.0
    R_vhc_count = R_vhc_count.sort_values(by=['region_cd','manufacture_date']).reset_index(drop=True)
    R_vhc_count['region_cd'] = R_vhc_count['region_cd'].astype(int)
    R_vhc_count = pd.merge(R.loc[:,['manufacture_date','region_cd','road_type','vkt_split_county']],
                           R_vhc_count, 
                           left_on = ['manufacture_date','region_cd'],
                           right_on = ['manufacture_date','region_cd'], how='left')
    
    R_vhc_count.loc[:,ADvhc] = R_vhc_count.loc[:,ADvhc].apply(lambda x: np.asarray(x) * \
                               R_vhc_count.vkt_split_county.values).fillna(0.0)
    R_vhc_count.set_index(['manufacture_date','region_cd','road_type'], inplace=True)
    R_vhc_count = R_vhc_count.sort_index()
    R.set_index(['manufacture_date','region_cd','road_type'], inplace=True)
    R = R.sort_index()
    R.loc[:,ADvhc] = R.loc[:,ADvhc].mul(R_vhc_count.loc[:,ADvhc], axis='columns', level='label')
    R.loc[:,ADvhc] = R.loc[:,ADvhc].div(12.35)
    R = R.reset_index(drop=False)
    
    Sfi = aux_EE_AD.copy()
    Sfi.loc[:,ADvhc] = Sfi.loc[:,ADvhc].mul(0.7 / 12.35)
#    Sfi_vhc_count = AD_yr_vhc.vhc_count.copy()
#    Sfi_vhc_count.loc[:,list_vhc_no_gas] = Sfi_vhc_count.loc[:,list_vhc_no_gas] * 0.0
#    Sfi_vhc_count['region_cd'] = Sfi_vhc_count['region_cd'].astype(int)
#    Sfi_vhc_count = Sfi_vhc_count.sort_values(by=['region_cd','manufacture_date']).reset_index(drop=True)
#    Sfi_vhc_count = pd.merge(Sfi.loc[:,['manufacture_date','region_cd','road_type','vkt_split_county']],
#                             Sfi_vhc_count, 
#                             left_on = ['manufacture_date','region_cd'],
#                             right_on = ['manufacture_date','region_cd'], how='left')
#    
#    Sfi_vhc_count.loc[:,ADvhc] = Sfi_vhc_count.loc[:,ADvhc].apply(lambda x: np.asarray(x) * \
#                                 Sfi_vhc_count.vkt_split_county.values).fillna(0.0) 
#    Sfi_vhc_count.set_index(['manufacture_date','region_cd','road_type'], inplace=True)
#    Sfi_vhc_count= Sfi_vhc_count.sort_index()
    Sfi.set_index(['manufacture_date','region_cd','road_type'], inplace=True)
    Sfi = Sfi.sort_index()
#    Sfi.loc[:,ADvhc] = Sfi.loc[:,ADvhc].mul(Sfi_vhc_count.loc[:,ADvhc], axis='columns', level='label')
...
#    total_poll_WGT = total_poll_WGT.drop(columns=['region_cd'])
    ...
    total_county_2CVS = total_county.drop(columns = ['geometry','region_name_SK'])
    total_county_2CVS.to_csv(output_dir+'/'+'County_Total_Emissions_Tons_per_Year.csv', sep=',', index=False)
    
    total_county_WGT_2CSV = total_county_WGT.drop(columns = ['geometry','region_name_SK','vkt_avg','vkt_weight'])
    total_county_WGT_2CSV.to_csv(output_dir+'/'+'County_Total_Normalized_Emissions_Tons_per_Year.csv', sep=',', index=False)
...
for ipol in grd_all.pollutant.unique():
    grd_aux = grd_all.loc[grd_all.pollutant.isin( [ipol,0]) & (grd_all.grid_id.isin(grid_id_list))].reset_index(drop=True)
    grd_aux.loc[:,'pollutant'] = ipol
    gridded_emissions = gridded_emissions.append(grd_aux, ignore_index=True, sort=False)
...
if plot_figures == 'yes':
    ...
    for ipol in polls:
        ...
        ax.set_title('Total Emissions of Country-{0} [ton/yr]'.format(ipol), fontsize = 22)
        ...
        #    ax.axis('off')
        name = 'Country_link_emis_SK_Total_emis_Fig_ax_{0}.png'.format(ipol)
    ...
    for ipol in polls:
        ...
        cbar = fig.colorbar(sm)
        #    ax.axis('off')
        name = 'Country_county_emis_SK_Total_emis_{0}.png'.format(ipol)
        fig.savefig(output_dir+'/plots/'+name, bbox_inches = 'tight',dpi=300)
        plt.close()
    ...
    for ipol in polls:
        ...
        for ifuel in fuels:  #make a correction on the columns extraction to separate h-gasoline from gasoline
            fuel_df = pol_df.loc[:,pol_df.columns.str.contains( )]
    ...
    for ipol in polls:
        ...
        ax.set_title('Country Grid Emissions - {0}'.format(ipol), fontsize = 22)
        sm = plt.cm.ScalarMappable(cmap=GrayJet, 
        ...
        name = 'Country_Grid_emis_SK_Total_emis_{0}.png'.format(ipol)
...
if plot_24 == 'yes':
	...
	for ipol in polls:
		...
		for itime in run_period.DateTime.loc[0:24]:
			...
			ax.set_title('Grid Emissions of Country - {0}'.format(ipol), fontsize = 22)
			sm = plt.cm.ScalarMappable(cmap=GrayJet, 
									   norm=plt.Normalize(vmin=vmin,
														  vmax=vmax))
			...
			#    ax.axis('off')
			if itime.hour < 10:
				name = 'Hourly_Country_Grid_emis_SK_Total_emis_{0}_hour_0{1}.png'.format(ipol,itime.hour)
			else:
				name = 'Hourly_Country_Grid_emis_SK_Total_emis_{0}_hour_{1}.png'.format(ipol,itime.hour)
